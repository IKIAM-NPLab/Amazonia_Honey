...
Correlation between feature abundance and antibacterial result (zone inhibition diameter) using the PCA and Pearson correlation aproaches.
...
corr_pca_plot
# Save plot
#ggsave('Result/notame_Result/HS_GCMS/corr_pca_plot.pdf', width = 7, height = 4, #device='pdf', dpi="print")
...
corr_load_plot
# Save plot
#ggsave('Result/notame_Result/HS_GCMS/corr_load_plot.pdf', width = 14, height = 7, #device='pdf', dpi="print")
...
Correlation between antimicrobial activity and feture abundance.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}

# Drop QCs
no_qc <- drop_qcs(glog_set)
# Convert antimicrobial result to numeric
no_qc$K_pneumonia_75 <- as.numeric(no_qc$K_pneumonia_75)
no_qc$P_mirabilis_50 <- as.numeric(no_qc$P_mirabilis_50)
no_qc$P_mirabilis_75 <- as.numeric(no_qc$P_mirabilis_75)
no_qc$P_mirabilis_100 <- as.numeric(no_qc$P_mirabilis_100)
# correlation of feature with the antimicrobial result
antimicrobial <- perform_correlation_tests(no_qc,
...
Plot of correlation result.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}

# Adding row names to the correlation result table
antimicrobial$Feature_ID <- antimicrobial$X
# Log10 transformation of p-value
antimicrobial$log10_P <- -log10(antimicrobial$Correlation_P)
# Keep metabolites with positive correlation r > 0
antimicrobial <- subset(antimicrobial, Correlation_coefficient > 0.5)
# Adding metabolite names to the correlation result table
antimicrobial_compouds <- left_join(meta_table, antimicrobial)


corr_plot <- ggplot(antimicrobial, aes(Correlation_coefficient, log10_P,
                                       color = Y, shape = Y)) +
  geom_point(size = 2, alpha = 0.4) +
  #ggrepel::geom_label_repel(data = antimicrobial_compouds,
  #                          aes(label = meta_table$Metabolite),
  #                          color = "black",
  #                          box.padding = 0.37,
  #                          label.padding = 0.22,
...
e_coli <- read_excel("Antimicrobial_of_honey.xlsx", sheet = 1)

#perform shapiro-wilk test (p-value = 0.007755)
shapiro.test(e_coli$E_coli_100)
#perform shapiro-wilk test (p-value = 0.02356)
shapiro.test(e_coli$E_coli_75)




s_aureus <- read_excel("Antimicrobial_of_honey.xlsx", sheet = 2)

#perform shapiro-wilk test (p-value = 6.413e-05)
shapiro.test(s_aureus$S_aureus_25)
multi.hist(s_aureus$S_aureus_25, global = FALSE)
#perform shapiro-wilk test (p-value = 0.007736)
shapiro.test(s_aureus$S_aureus_50)

#perform shapiro-wilk test (p-value = 0.009524)
shapiro.test(s_aureus$S_aureus_75)

#perform shapiro-wilk test (p-value = 0.01106)
shapiro.test(s_aureus$S_aureus_100)




k_pneumonia <- read_excel("Antimicrobial_of_honey.xlsx", sheet = 3)

#perform shapiro-wilk test (p-value = 0.2401)
shapiro.test(k_pneumonia$K_pneumonia_75)




p_mirabilis <- read_excel("Antimicrobial_of_honey.xlsx", sheet = 4)

#perform shapiro-wilk test (p-value = 0.07623)
shapiro.test(p_mirabilis$P_mirabilis_50)

#perform shapiro-wilk test (p-value = 0.1249)
shapiro.test(p_mirabilis$P_mirabilis_75)

#perform shapiro-wilk test (p-value = 0.05333)
shapiro.test(p_mirabilis$P_mirabilis_100)
